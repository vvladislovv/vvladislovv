local CityGame = game.Workspace.Games:WaitForChild("City")
local PromtProxi = CityGame:WaitForChild("Proxi"):WaitForChild("ProximityPrompt")
local TeleportServis = game:GetService("TeleportService")
local Player = game:GetService("Players").LocalPlayer	
local mFloor = math.floor
local Stop = 0 
local BillboardGui = game.Workspace.Games.City.TimerCity:WaitForChild("BillboardGui")

local Teleporting = script.Parent.Frame
local TextTeleporting = Teleporting.TextLabel
local CouldTime = task.wait(0.3)

local ObbyPlaycdId = 14660174723
...
function GuiStart()
	...
	--TeleportServis:Teleport(WaspPlaycdId, Player)
end

while true do task.wait(1)
	task.wait()
	local Time = 1719752400 - tick	() 
